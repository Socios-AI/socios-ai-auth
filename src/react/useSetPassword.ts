...
export function useSetPassword(opts: UseSetPasswordOptions = {}): {
  state: "initial" | "ready" | "submitting" | "success" | "error";
  errorCode?: AuthErrorCode;
  resolvedType?: "invite" | "recovery";
  submit: (newPassword: string) => Promise<void>;
} {
  const tc = useTokenConsumption({ acceptedTypes: ["invite", "recovery"] });
  ...
  const resolvedType = tc.state === "ready" ? (tc.resolvedType as "invite" | "recovery") : undefined;
  ...
  if (submit === "submitting") return { state: "submitting", resolvedType, submit: doSubmit };
  if (submit === "success") return { state: "success", resolvedType, submit: doSubmit };
  if (submit === "lastError") return { state: "ready", errorCode: submitErr, resolvedType, submit: doSubmit };
  return { state: "ready", resolvedType, submit: doSubmit };
}
